...
public class ChatServiceImpl implements ChatService {
    ...
    @Override
    @Transactional
    public Slice<ConversationResponse> getUserConversations(User user, Pageable pageable) {
        ...
        if (isCandidateSender) {
            ...
            return conversationRepository.findCandidateConversations(candidate, pageable).map(
                    conversation -> {
            ...
            return conversationRepository.findCompanyConversations(company, pageable).map(
                    conversation -> {
                        conversation.setPartnerAvatarUrl(
                                s3Service.generatePresignedUrl(conversation.getPartnerAvatarUrl()));
                        String lastMessage = conversation.getLastMessage();
